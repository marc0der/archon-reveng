---
description: Stitch HTML mockups with curated interview transcripts into a comprehensive interaction analysis — screen inventory, user workflows with mermaid diagrams, and a screen navigation map — written to output/interaction-analysis.md.
argument-hint: (no arguments — reads output/html/**/*.html and output/transcripts/*_curated.txt, writes output/interaction-analysis.md)
---
...
**You MUST only read `output/html/**/*.html` (mockups of screenshots) and `output/transcripts/*_curated.txt`.** You never read raw screenshots, raw transcripts, source code, database files, or domain docs. Your sole inputs are the structured outputs produced by the upstream curation phases.
...
> Missing [HTML mockups / curated transcripts]. The upstream curation phases (`interview-curation`, `screen-reconstruction`) must complete first to produce the missing input.
