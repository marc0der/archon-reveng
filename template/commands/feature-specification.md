---
description: Write one detailed feature specification per FT-NNN feature in the confirmed feature plan — spawning a feature-writer worker (the node's inline agents: sub-agent) per feature, in parallel, each writing output/features/FT-NNN-<slug>.md from verbatim PRD content and the shared authoring template.
argument-hint: (no arguments — reads output/feature-plan.md and output/PRD.md, writes output/features/FT-*.md)
---

# Feature Specification

You are a **feature specification orchestrator** for legacy application reverse-engineering. The feature plan has already been decomposed and **approved at the feature-plan gate**. Your task is to turn each planned feature into a full, self-contained feature specification file, written by a dedicated **feature-writer** worker.

You do **not** write the feature files yourself. For each feature you assemble a fully self-contained prompt and spawn one `feature-writer` worker — defined as this node's inline sub-agent (the workflow node's `agents:` block) and invoked through the **Task** tool. The workers run **in parallel**, one per feature, each writing a single `output/features/FT-NNN-<slug>.md`.

Use British English in all output. Stay stack-agnostic — describe the re-engineered system, not a particular language or framework.

## Hard constraints — inputs

- **`output/feature-plan.md`** is your authoritative list of features. It was produced by `feature-decomposition` and **confirmed by the human at the feature-plan gate** — the ids, layers, priorities, and dependency columns are fixed. Do **not** re-plan, re-number, or re-layer the features, and do **not** wait for further confirmation: the gate already owns that decision.
- **`output/PRD.md`** is the source of the verbatim content each worker needs. You read it to extract each feature's relevant sections and the shared context, but you never write to it.
- Do **not** read raw sources (`src/`, `transcripts/`, `screenshots/`) or the upstream analysis files — the confirmed plan and the PRD are your only inputs.

## Execution sequence

Work through these steps in order.

### Step 1: Validate the inputs exist

Use the Read tool to open `output/feature-plan.md`. If it does not exist, stop and tell the user:

> Missing feature plan at `output/feature-plan.md`. The **feature-decomposition** phase produces it and it must be approved at the feature-plan gate before features can be specified.

Then use the Read tool to open `output/PRD.md`. If it does not exist, stop and tell the user:

> Missing PRD at `output/PRD.md`. The **requirements-synthesis** phase produces it; the feature specifications derive their verbatim content from it.

If either input is missing, produce no output files.

### Step 2: Skip features already written (idempotency)

Use Glob for `output/features/FT-*.md`. For every `FT-NNN` that already has a file, **skip it** — do not regenerate features that already exist. Only spawn workers for features in the plan that have no corresponding file yet. If every feature in the plan already has a file, write nothing, report that all features are already specified, and stop.

This makes the phase safe to re-run and resume.

### Step 3: Read and internalise the plan and the PRD

Read the entire feature plan table and, for each feature, note its:

- **Feature ID** (`FT-NNN`) and **Title**
- **MoSCoW priority**
- **Build layer**
- **PRD sections** it derives from
- **Upstream dependencies** and **Downstream dependencies** (by feature id, or "None")

Then read the entire PRD. Hold in context the **shared PRD content** that applies across all features — the actors/personas table, the glossary, and any global business rules not specific to a single feature. Every worker receives this shared context verbatim so the feature boundaries stay clean.

### Step 4: Ensure the output directory exists

Run `mkdir -p output/features`.

### Step 5: Spawn one feature-writer worker per feature, in parallel

For each feature **not already written** (per Step 2), launch a `feature-writer` worker using the **Task** tool with `subagent_type: feature-writer` (the inline sub-agent declared on this node). **Fire all the workers in a single message** — do not wait for one to finish before launching the next.

Each `feature-writer` prompt must be **fully self-contained** — the worker reads nothing from disk; everything it needs is in the prompt. Construct each prompt with the following clearly-labelled sections.

**Feature metadata:**
- Feature ID (`FT-NNN`) — exactly as in the plan; never reassign it
- Feature title
- MoSCoW priority
- Output file path: `output/features/FT-NNN-<slug>.md` (lowercase, hyphenated slug of the title)
- Upstream feature ids (or "None")
- Downstream feature ids (or "None")
- First user story id: the globally sequential `US-XXX` number this feature starts from — calculate it by summing the estimated story counts of all lower-numbered features, so story ids never collide across features

**Feature-specific PRD content:**
Paste the **verbatim** text of every PRD section relevant to this feature — its bounded context definition, the relevant screens, the relevant workflows, the relevant business rules, the relevant entities and attributes, and the relevant legacy pain points. Use the plan's *PRD sections* column to locate them. Do not summarise — extract and paste the actual PRD text.

**Shared PRD context:**
Paste the following from the PRD **verbatim** into every worker prompt — this is what keeps feature boundaries clean:
- The full actors/personas table
- The full glossary
- Any global business rules that are not specific to one feature

**Template and authoring rules:**
Paste the entire **Feature template and authoring rules** section from the end of this command **verbatim** into the prompt. Do not summarise, abridge, or omit any part — including the wireframe rules and the full markdown template. This is what the worker follows to produce the file; without it inlined, the worker has nothing reliable to follow.

### Step 6: Report results

Once every worker has finished, return a short summary containing:
- The number of feature files generated this run (and the number skipped as already present)
- The file path of each feature written
- The total number of user stories across all features
- Any open questions or gaps the workers surfaced

---

## Feature template and authoring rules

The content below is what you paste verbatim into every feature-writer prompt under the **Template and authoring rules** heading (per Step 5). Do not paraphrase, summarise, or trim — copy it as-is, starting from "How to fill the template" through the end of the markdown template.
